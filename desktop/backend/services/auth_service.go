...
func (a *AuthService) recoverFromCrash(cfg *SharedConfig) {
	files := []string{
		...
		filepath.Join(cfg.ConfigDir, "ns-drive.db"),
	}
...
// encryptConfigFiles encrypts rclone.conf and ns-drive.db
...
// decryptConfigFiles decrypts rclone.conf.enc and ns-drive.db.enc
...
func (a *AuthService) cleanupEncryptedFiles(cfg *SharedConfig) {
	encFiles := []string{
		...
		filepath.Join(cfg.ConfigDir, "ns-drive.db.enc"),
	}
