# GN Drive Documentation

Welcome to the GN Drive documentation. This directory contains detailed technical documentation for developers and contributors.

## Documentation Index

| Document | Description |
|----------|-------------|
| [ARCHITECTURE.md](ARCHITECTURE.md) | System architecture, data flow, and design patterns |
| [SECURITY.md](SECURITY.md) | Password protection, encryption, and rate limiting |
| [DEVELOPMENT.md](DEVELOPMENT.md) | Development setup, commands, and debugging |
| [CONTRIBUTING.md](CONTRIBUTING.md) | Contribution guidelines and code standards |
| [API.md](API.md) | Backend services API reference |
| [EVENTS.md](EVENTS.md) | Event system documentation |

## Quick Links

- [Main README](../README.md) - Project overview and quick start
- [Backend README](../desktop/backend/README.md) - Backend structure
- [Frontend README](../desktop/frontend/README.md) - Frontend structure

## Getting Started

1. **New to GN Drive?** Start with the [main README](../README.md) for installation and quick start
2. **Want to contribute?** Read [CONTRIBUTING.md](CONTRIBUTING.md)
3. **Understanding the architecture?** See [ARCHITECTURE.md](ARCHITECTURE.md)
4. **Setting up development?** Check [DEVELOPMENT.md](DEVELOPMENT.md)

## Technology Stack

- **Backend**: Go 1.25 + Wails v3
- **Frontend**: Angular 21 + Tailwind CSS
- **Cloud Sync**: rclone integration
- **Build**: Taskfile

## Support

- Report issues on GitHub
- Check rclone documentation for cloud provider setup
